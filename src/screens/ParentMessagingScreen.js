...
const ParentMessagingScreen = ({ navigation, route }) => {
  const { theme, fontSizes } = useTheme();
  const { t } = useLanguage();
  const { refreshUnreadCounts } = useMessaging();
  const { authCode, parentName } = route.params;

  console.log('🔍 PARENT MESSAGING SCREEN - Received params:', {
    authCode,
    parentName,
  ...
  // Helper function to check if parent is a member of the conversation
  const isParentMember = useCallback(
    (conversation, currentUser) => {
      ...
      const isMember = membersToCheck.some((member) => {
        ...
        const matches = {
          ...
          currentUserParentId:
            currentUser && member.id === currentUser.parent_id,
          currentUserName: currentUser && member.name === currentUser.name,
          currentUserEmail: currentUser && member.email === currentUser.email,
          parentNameParam: member.name === parentName,
          parentTypeAndName:
            member.user_type === 'parent' && member.name === parentName,
        };

        // Check various possible ID and name matches
        const isMatch =
          matches.currentUserId ||
          matches.currentUserParentId ||
          matches.currentUserName ||
          matches.currentUserEmail ||
          matches.parentNameParam ||
          matches.parentTypeAndName;

        // Additional fallback: For testing purposes, if no exact match is found
        // and we have a parent name parameter, match the first parent in the conversation
  ...
  // Fetch conversations and filter for parent
  const fetchConversations = useCallback(async () => {
    try {
      setLoading(true);
      console.log('📱 PARENT MESSAGING: Using authCode:', authCode);
      ...
      if (response.success && response.data) {
        ...
        // Since we're using the parent's authCode, the API should return only relevant conversations
        // But we'll keep some filtering as a safety measure
        const parentConversations = allConversations
          .filter((conversation) => {
            // If we have user data, use strict filtering
            if (currentUser) {
              return isParentMember(conversation, currentUser);
            }

            // Fallback: if no user data but we have a parent name, include conversations with parents
            // This helps with testing and edge cases
            if (parentName) {
              const hasParents =
                conversation.members?.some((m) => m.user_type === 'parent') ||
                conversation.grouped_members?.some(
                  (g) => g.type === 'parent' && g.count > 0
                );
              return hasParents;
            }

            // Default: include all conversations (since we're using parent's authCode)
            return true;
          })
        ...
        console.log(
          `📱 PARENT MESSAGING: Using parent authCode - Found ${allConversations.length} total conversations, ${parentConversations.length} for parent`
        );

        setConversations(parentConversations);
      }
    } catch (error) {
  ...
  }, [authCode, parentName]); // Removed function dependencies to prevent infinite loops
  ...
  // Handle search and filter results for parent
  const handleSearch = useCallback(
    async (query) => {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          // Filter search results to show only conversations where parent is a member
          const originalConversations = response.data.conversations || [];
          const filteredConversations = originalConversations
            .filter((conversation) => isParentMember(conversation, currentUser))
            .map((conversation, index) => ({
              ...conversation,
              // Ensure each conversation has a unique identifier
              conversation_uuid:
          ...
          console.log(
            `📱 PARENT SEARCH: Found ${originalConversations.length} search results, ${filteredConversations.length} for parent`
          );
    ...
    [authCode, getCurrentUserData, isParentMember]
  );
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyState}>
      ...
      <Text style={styles.emptyStateText}>
        You can start conversations with your children's teachers and school
        staff using the + button above
      </Text>
    </View>
  );
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener('focus', () => {
      ...
      if (now - lastNavigationRefresh.current < 1000) {
        console.log(
          '🔍 PARENT MESSAGING: Skipping navigation refresh - too soon'
        );
        return;
      }
      ...
      console.log(
        '🔍 PARENT MESSAGING: Navigation focus - refreshing conversations'
      );
